...
1. **Шапка шага подтверждения:**
   - ✅ Убрана верхняя полоска прогресса («Финальный шаг», «5 из 5», прогресс-бар). Оставлены только индикатор шагов с точками (`StepIndicator`) и заголовок «Подтверждение записи» с кнопкой «Назад».

2. **Чекбокс и кнопка:**
   - ✅ Чекбокс «Я согласен с Условиями обслуживания и Политикой отмены» сделан интерактивным: без галочки кнопка «Записаться» неактивна, после установки галочки — активна.
   - ✅ Кнопка переименована в «Записаться» и снова показывает цену: слева «Записаться», справа сумма в рублях (например «5 000 ₽»).

3. **Ссылки «Условия обслуживания» и «Политика отмены»:**
   - ✅ Если в настройках заданы URL, в тексте рядом с чекбоксом эти фразы отображаются как кликабельные ссылки (открываются в новой вкладке). Если URL не заданы — обычный текст.

4. **Настройки в админке:**
   - ✅ В разделе «Настройки» → «Записи» добавлен блок «Ссылки на экран подтверждения записи»: поля «Условия обслуживания (URL)» и «Политика отмены (URL)». Значения сохраняются через API и подставляются в веб-приложение.

5. **Бэкенд:**
   - ✅ В `SettingsService` добавлены `getBookingTermsSettings()` и `setBookingTermsSettings()` (ключ `bookingTerms`).
   - ✅ Админ: `GET/PUT /settings/booking-terms`. Публичный эндпоинт для веб-приложения (без авторизации): `GET /public/settings/booking-terms` — возвращает `{ termsOfServiceUrl, cancellationPolicyUrl }` (`PublicSettingsController`).

**Файлы изменены:**
- `apps/telegram/src/features/appointments/ConfirmBooking.tsx` — убрана полоска в шапке, интерактивный чекбокс, кнопка «Записаться» с ценой, кликабельные ссылки из настроек.
- `apps/telegram/src/shared/api/settings.ts` — API `getBookingTerms()` для публичного эндпоинта.
- `backend/src/modules/settings/settings.service.ts` — get/set BookingTermsSettings.
- `backend/src/modules/settings/settings.controller.ts` — GET/PUT booking-terms (админ), GET /settings возвращает bookingTerms.
- `backend/src/modules/settings/public-settings.controller.ts` — публичный GET booking-terms.
- `backend/src/modules/settings/settings.module.ts` — регистрация PublicSettingsController.
- `admin/app/settings/page.tsx` — блок «Ссылки на экран подтверждения записи», загрузка/сохранение bookingTerms.
...
1. **Звёзды рейтинга с поддержкой половинки (например 4.5 из 5):**
   - ✅ **Telegram Web App** (`apps/telegram/src/features/services/MasterSelection.tsx`): функция `renderStars` переработана — после полных звёзд выводится половинка (иконка `star_half` Material Symbols) при остатке 0.25–0.75.
   - ✅ **Админка** (`admin/app/masters/page.tsx`): добавлен компонент `StarRating` с поддержкой половинки (полная звезда + clip-path для половины). Используется в карточке мастера (рейтинг) и в блоке отзывов (рейтинг каждого отзыва).

2. **Кликабельные отзывы:**
   - ✅ **Админка:** В блоке «Последние отзывы» у мастера — кнопка «Все отзывы (N)» открывает модальное окно со списком одобренных отзывов; каждый отзыв кликабелен и открывает второе модальное окно с полным текстом отзыва. Исправлена загрузка отзывов: используется `data?.data ?? []` из ответа API.
   - ✅ **Telegram Web App:** Кнопка «Отзывы» у мастера открывает модальное окно со списком отзывов по этому мастеру. **Просмотр отзывов доступен всем.** В модальном окне каждый отзыв кликабелен — открывается детальный просмотр (полный комментарий).
   - ✅ **Telegram-бот:** В команде `/masters` под списком мастеров добавлены кнопки «💬 Отзывы — [Имя мастера]». **Просмотр отзывов доступен всем.** Обработчик `reviews_master:masterId` загружает одобренные отзывы по мастеру и отправляет сообщение со списком (рейтинг с половинкой в виде ⭐½, имя автора, комментарий до 120 символов). Оставить отзыв можно только после посещения услуги (проверка при создании отзыва на бэкенде).

3. **Фиксированная шапка и одна статичная кнопка «Выбрать»:**
   - ✅ **Выбор мастера (Web App):** Шапка остаётся `sticky top-0`. Список мастеров прокручивается; у каждой карточки убрана отдельная кнопка «Выбрать». Внизу экрана закреплена одна общая кнопка «Выбрать» (фиксированный блок). Выбор мастера — по клику на карточку (подсветка рамкой); при включённом «Любой свободный мастер» кнопка «Выбрать» ведёт на календарь с первым мастером из списка.
   - ✅ **Выбор услуги (Web App):** Шапка и прогресс-бар уже были sticky; футер с кнопкой «Продолжить» — fixed. Список услуг прокручивается. Удалены отладочные `console.log` из страницы услуг.

**Файлы изменены:**
- `apps/telegram/src/features/services/MasterSelection.tsx` — половинки звёзд, модальные окна отзывов, проверка завершённых записей, одна кнопка «Выбрать», прокручиваемый список.
- `apps/telegram/src/features/services/page.tsx` — удалены логи отладки категорий.
- `admin/app/masters/page.tsx` — компонент `StarRating`, кликабельные отзывы, модалки «Все отзывы» и детальный просмотр отзыва.
- `backend/src/modules/telegram/telegram-bot.service.ts` — callback `reviews_master:masterId`, кнопки «Отзывы» в `/masters`, метод `handleMasterReviewsRequest`, импорт `ReviewStatus`.

**Ранее уже было сделано (из предыдущего контекста):**
- Исправление отображения услуг в категориях в бэкенде и веб-приложении.
- Дизайн окна выбора мастера в соответствии с макетом (фильтры, чекбокс «Любой свободный мастер», карточки с рейтингом и кнопкой «Отзывы»).

✅ **Отзывы: просмотр всем, оставить — после посещения; бонусы за отзыв; раздел «Отзывы» в админке (текущая сессия):**

**Выполнено:**

1. **Просмотр отзывов всегда доступен; оставить отзыв — только после посещения услуги:**
   - ✅ **Telegram Web App:** Убрана проверка «прошёл процедуру» для просмотра отзывов. Кнопка «Отзывы» всегда активна и открывает модальное окно с отзывами по мастеру.
   - ✅ **Telegram-бот:** Убрана проверка наличия завершённой записи при нажатии «Отзывы» в `/masters`. Просмотр отзывов доступен всем. Оставить отзыв по-прежнему можно только по завершённой записи (логика на бэкенде в `reviewsService.create`).

2. **Бонусы за отзыв:**
   - ✅ **Настройка в админке:** В разделе «Настройки» → «Бонусы» добавлено поле «Баллов за отзыв». Значение сохраняется в настройках бонусной системы (`pointsForReview`).
   - ✅ **Бэкенд:** В `getBonusSettings` / `setBonusSettings` добавлено поле `pointsForReview` (по умолчанию 0). При одобрении отзыва модератором (`reviewsService.moderate` со статусом `approved`) клиенту начисляются бонусы за отзыв (если бонусная система включена и `pointsForReview` > 0) через `FinancialService.awardBonusPoints` с описанием «Бонусы за отзыв». Начисление выполняется только при переходе статуса в «одобрен» (не при повторном сохранении уже одобренного отзыва).
   - ✅ **ReviewsModule** импортирует `FinancialModule` и `SettingsModule`; `ReviewsService` использует `FinancialService` и `SettingsService`.

3. **Модерация отзывов перед публикацией:**
   - ✅ Отзывы создаются со статусом `pending`. Публикуются только после одобрения админом (`POST /reviews/:id/moderate` с `status: approved`). В админке в разделе «Отзывы» для отзывов «На модерации» доступны кнопки «Одобрить» и «Отклонить» с опциональным комментарием модератора.

4. **Раздел «Отзывы» в админке:**
   - ✅ Новая страница `/reviews` (`admin/app/reviews/page.tsx`): таблица отзывов с колонками — клиент (кто оставил), услуга, мастер, оценка, комментарий, статус, дата. Фильтры: статус (все / на модерации / одобрены / отклонены), мастер, услуга. Пагинация.
   - ✅ Действия: **Одобрить** / **Отклонить** (модальное окно с комментарием модератора), **Редактировать** (оценка 1–5 и комментарий), **Удалить** (с подтверждением). После редактирования или удаления пересчитывается рейтинг мастера.
   - ✅ В сайдбар добавлен пункт «Отзывы» (иконка Star), маршрут `/reviews`.

5. **API отзывов (бэкенд):**
   - ✅ `GET /reviews/:id` — получение одного отзыва (с связями user, master, service).
   - ✅ `PATCH /reviews/:id` (только админ) — обновление оценки и/или комментария.
   - ✅ `DELETE /reviews/:id` (только админ) — удаление отзыва.
   - ✅ В `reviews.service` добавлены методы `findOne`, `update`, `remove`.

**Файлы изменены:**
- `apps/telegram/src/features/services/MasterSelection.tsx` — убрана проверка hasCompletedAppointment для просмотра отзывов.
- `backend/src/modules/telegram/telegram-bot.service.ts` — убрана проверка завершённой записи в `handleMasterReviewsRequest`.
- `backend/src/modules/settings/settings.service.ts` — добавлены `pointsForReview` в getBonusSettings/setBonusSettings, чтение `referralBonus` для обратной совместимости.
- `backend/src/modules/settings/settings.controller.ts` — в GET/PUT bonuses добавлено поле `pointsForReview`.
- `backend/src/modules/reviews/reviews.module.ts` — импорт FinancialModule, SettingsModule.
- `backend/src/modules/reviews/reviews.service.ts` — инъекция FinancialService, SettingsService; в moderate() начисление бонусов при одобрении; методы findOne, update, remove.
- `backend/src/modules/reviews/reviews.controller.ts` — эндпоинты GET :id, PATCH :id, DELETE :id.
- `admin/app/settings/page.tsx` — в настройках бонусов добавлено поле «Баллов за отзыв» (pointsForReview).
- `admin/app/components/Sidebar.tsx` — пункт меню «Отзывы» (Star).

**Файлы созданы:**
- `admin/app/reviews/page.tsx` — страница списка отзывов с фильтрами, модерацией, редактированием и удалением.
