...
✅ **Исправление экрана «Запись подтверждена!» — устранение «запись не найдена» после успешной записи:**

**Проблема:** После нажатия «Записаться» показывалось «Запись успешно», но затем экран успеха не загружался и отображалось «Не удалось загрузить данные записи» / «запись не найдена» — запрос GET /appointments/:id после перехода мог падать (401/403/404 или потеря state).

**Выполнено:**

1. **Бэкенд (`appointments.service.ts`):**
   - Метод `create()` после сохранения записи возвращает полную запись с связями: вызов `findById(savedAppointment.id, userId)` перед `return`, чтобы в ответе были **master, service, extraServices**.
   - В ответе POST на создание записи клиент сразу получает все данные для экрана «Запись подтверждена!».

2. **Фронт:**
   - **ConfirmBooking:** при успешном создании переход на `/booking-success/${id}` с передачей в **state** созданной записи: `navigate(\`/booking-success/${id}\`, { state: { appointment: created } })`.
   - **BookingSuccess:** при наличии **location.state.appointment** экран использует эти данные и не выполняет GET /appointments/:id сразу после создания; запрос по id выполняется только при отсутствии state (например, при обновлении страницы).
   - Добавлена обработка ошибок загрузки записи (retry: false, сообщение «Не удалось загрузить данные записи» и кнопка «В профиль»), разбор ответа API в `appointmentsApi.getById` (поддержка обёртки `res.data?.data ?? res.data`).

**Настройки подтверждения записи:** В админке (Настройки → Записи) переключатель «Подтверждение записи вручную»: включено — записи остаются в ожидании подтверждения админом; выключено — в Telegram-боте записи могут автоматически подтверждаться. На создание записи в веб-приложении и отображение экрана «Запись подтверждена!» эта настройка не влияет.

**Файлы изменены:**
- `backend/src/modules/appointments/appointments.service.ts` — create() возвращает findById() с полными связями.
- `apps/telegram/src/features/appointments/ConfirmBooking.tsx` — передача created в state при переходе на booking-success.
- `apps/telegram/src/features/appointments/BookingSuccess.tsx` — использование appointmentFromState, запрос по id только при отсутствии state, обработка ошибки загрузки.
- `apps/telegram/src/shared/api/appointments.ts` — getById: разбор ответа res.data?.data ?? res.data.
