...
**Исправление ошибки SchedulerService (в работе):**
- 🔄 Проблема: TypeORM пытается выбрать колонку referralCode через неправильный алиас (camelCase вместо snake_case)
- 🔄 Решение: Использование QueryBuilder вместо find с relations, добавлены явные имена колонок в декоратор @Column
- 🔄 Файлы изменены: `backend/src/entities/user.entity.ts` - добавлены name: 'referral_code' и name: 'referred_by_user_id', `backend/src/tasks/scheduler.service.ts` - использование QueryBuilder
- ❌ Ошибка все еще присутствует: TypeORM продолжает использовать camelCase при формировании SQL запросов, изменения не попали в скомпилированный JS файл
- ⚠️ Временное решение: Backend работает, но SchedulerService не может обрабатывать напоминания о записях
