...
**Причина:**
- **PostgreSQL volume уже существует с другими учетными данными (самая частая причина)**
- Несоответствие паролей в .env файле
- Пользователь не создан в базе данных

**ВАЖНО:** PostgreSQL применяет переменные окружения `POSTGRES_USER` и `POSTGRES_PASSWORD` **только при первом запуске** (при инициализации пустого volume). Если volume уже существует, эти переменные **игнорируются**, и PostgreSQL использует учетные данные, которые были установлены при первом запуске.
...
- Проверит конфигурацию `.env` файла
- Создаст пользователя и базу данных, если их нет
- Обновит пароль пользователя, если он не совпадает
- Перезапустит backend после исправления
