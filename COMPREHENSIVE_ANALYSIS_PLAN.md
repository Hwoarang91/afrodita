# Комплексный план исправлений проекта Afrodita

**Дата анализа:** 23.01.2026  
**Метод:** Глубокий анализ кода с использованием Context7 и детальной проверки

## Инструкции по запуску

### Запуск через Docker

```bash
docker compose build
docker compose up -d
docker compose logs -f
docker compose down
```

### Локальный запуск

```bash
npm install
npm run dev
```

### Запуск в продакшен на выделенном сервере Ubuntu

После настройки переменных окружения и запуска через docker compose.

---

## Общая информация

**Назначение проекта:** Система управления массажным салоном с Telegram-ботом, админ-панелью и веб-приложением.

**Технологический стек:**
- Backend: NestJS, TypeORM, PostgreSQL, Redis, Telegram MTProto
- Admin: Next.js 14, React, TypeScript
- Landing: React, Vite, TypeScript
- Telegram App: React, Vite, TypeScript

**Архитектура:** Монолитный backend с модульной структурой, микросервисная архитектура для frontend приложений.

---

## КРИТИЧЕСКИЕ ПРОБЛЕМЫ (Приоритет 1)

### 1. Проблема с 2FA аутентификацией (PASSWORD_HASH_INVALID)

**Статус:** ✅ **ВЫПОЛНЕНО** — самописный SRP заменён на `checkPassword` из `@mtkruto/node`

**Текущее состояние:**
- ✅ В `verify2FAPasswordWithStored`: `ap = await client.invoke(account.getPassword)`; `input = await checkPassword(password, ap)`; `client.invoke(auth.checkPassword, { password: input })`. Самописный SRP (~250 строк) удалён.
- ✅ `tssrp6a` удалена из `package.json`.
- ⚠️ Тесты для 2FA процесса — по плану §8 (auth)

**Проверено:**
- ✅ `auth.service.ts`: используется `checkPassword` из `@mtkruto/node` (аналог client.computeCheck, корректная SRP по MTProto)
- ✅ Сборка backend проходит

**Решение:**
1. ✅ Использовать `checkPassword(password, ap)` из `@mtkruto/node` (account.getPassword → ap)
2. ✅ Удалить самописный код SRP вычислений
3. ⚠️ Добавить тесты для 2FA процесса (в рамках §8)

**Файлы для изменения:**
- `backend/src/modules/auth/auth.service.ts` (строки 1476-1674)

**Оценка:** 4-6 часов

---

### 2. Нарушение архитектурных принципов - использование message.includes() вне mapper

**Статус:** ✅ **ВЫПОЛНЕНО** — isRequire2faActionError в mapper, все проверки убраны из handler и auth.service.

**Текущее состояние:**
- ✅ `isRequire2faActionError` и др. проверки в `telegram-error-mapper.ts`; все `message.includes()` убраны из `mtproto-error.handler.ts` и `auth.service.ts`.

**Проверено:**
- ✅ mtproto-error.handler.ts — проверки через mapper
- ✅ auth.service.ts — использование mapper, без message.includes

**Решение:**
1. ✅ Переместить все проверки в `telegram-error-mapper.ts`
2. ✅ Использовать `isFatalTelegramError`, `isRetryableTelegramError`, `isRequire2faActionError` из mapper
3. ✅ Удалить все `message.includes()` из mtproto-error.handler и auth.service
4. ✅ Исправить auth.service для использования mapper

**Файлы для изменения:**
- `backend/src/modules/telegram/utils/mtproto-error.handler.ts`
- `backend/src/modules/auth/auth.service.ts`
- `backend/src/modules/telegram/utils/telegram-error-mapper.ts` (расширить)

**Оценка:** 2-3 часа

---

### 3. Отсутствие пагинации в критичных запросах

**Статус:** ✅ **ВЫПОЛНЕНО** — appointments, reviews с page/limit; notifications sendBroadcast .take(5000).

**Текущее состояние:**
- ✅ `appointments.service` — findAll с page/limit (PaginationDto, normalizePagination)
- ✅ `reviews.service` — findAll с page/limit
- ✅ `notifications.service` — sendBroadcast с .take(5000) для выборки пользователей

**Проверено:**
- ✅ appointments — пагинация
- ✅ reviews — пагинация
- ✅ notifications — лимит 5000 для рассылки

**Решение:**
1. ✅ Пагинация в appointments и reviews
2. ✅ Лимит в notifications.sendBroadcast
3. ✅ Валидация limit (PaginationDto, max 100) — см. §6

**Файлы для изменения:**
- `backend/src/modules/appointments/appointments.service.ts`
- `backend/src/modules/reviews/reviews.service.ts`
- `backend/src/modules/notifications/notifications.service.ts`
- DTO для пагинации (создать или использовать существующие)

**Оценка:** 3-4 часа

---

### 4. TypeScript strict mode отключен

**Статус:** ✅ **ЧАСТИЧНО ВЫПОЛНЕНО** — strict: true, strictNullChecks, noImplicitAny включены; сборка OK

**Текущее состояние:**
- `backend/tsconfig.json`: `strict: true`, strictNullChecks, noImplicitAny (сборка OK)
- ✅ `noFallthroughCasesInSwitch`, `noImplicitReturns`, `noUnusedLocals` включены

**Проверено:**
- ✅ strict: true — исправлены: `error` в catch → getErrorMessage/getErrorStack (auth.service, jwt.service, notifications.service, scheduler.service, telegram-session.guard); InvokeClient+приведение в invokeWithRetry (mtproto-retry.utils, telegram-session.guard, telegram-heartbeat, telegram-user-client×3); `return (await client.invoke(request)) as T` в mtproto-retry

**Решение:**
1. ✅ noFallthroughCasesInSwitch, noImplicitReturns, noUnusedLocals
2. ✅ strictNullChecks, noImplicitAny
3. ✅ strict: true — error.message/stack→getErrorMessage/getErrorStack; InvokeClient для @mtkruto Client; mtproto-retry return as T
4. Тесты — в последнюю очередь

**Файлы для изменения:**
- `backend/tsconfig.json`
- Файлы с ошибками при включении strictNullChecks/noImplicitAny

**Оценка:** 8-12 часов (частично)

---

### 5. Множественные использования типа `any`

**Статус:** ✅ **ВЫПОЛНЕНО** — options/keyboard и keyboard-массивы в telegram-bot типизированы

**Текущее состояние:**
- ✅ `sendMessage(chatId, message, options?)` → `Types.ExtraReplyMessage`; `sendMessageWithKeyboard(..., keyboard)` → `Types.ExtraReplyMessage`
- ✅ `sendPrivateReply(..., options?)` → `Types.ExtraReplyMessage`; `reply_to_message_id` при fallback → `as Types.ExtraReplyMessage`
- ✅ `sendPrivateCallbackReply(..., keyboard?, options?)` → `keyboard?: { reply_markup?: Types.ExtraReplyMessage['reply_markup'] }`, `options?: Types.ExtraReplyMessage`; `editMessageText` reply_markup → `as ExtraEditMessageText['reply_markup']`
- ✅ `keyboard: any[]` → `unknown[][]` (календарь, записи×2); `currentRow: any[]` → `unknown[]`; `keyboardButtons: any[]` → `unknown[][]`; на границе с Telegraf: `Markup.inlineKeyboard(keyboard as never)` (HideableIKBtn[][] не экспортируется)

**Проверено:**
- ✅ `backend/src/modules/telegram/telegram-bot.service.ts` — сборка с strict: true проходит

**Решение:**
1. ✅ entity, chatInfo, apt/appointment.*, role, availableMasters, user, promo, faq, selectedServices, servicesToBook
2. ✅ options/keyboard в sendMessage, sendMessageWithKeyboard, sendPrivateReply, sendPrivateCallbackReply — Types.ExtraReplyMessage и { reply_markup }
3. ✅ keyboard-массивы — unknown[][], unknown[]; на вызове Markup.inlineKeyboard — `as never`

**Файлы для изменения:**
- `backend/src/modules/telegram/telegram-bot.service.ts`

**Оценка:** 4-6 часов (большая часть выполнена)

---

### 6. Отсутствие валидации максимального значения limit

**Статус:** ✅ **ВЫПОЛНЕНО** — PaginationDto, normalizePagination (max 100) в users, services, masters, appointments, reviews.

**Текущее состояние:**
- ✅ PaginationDto и `normalizePagination(..., { max: 100 })` в users, services, masters, appointments, reviews.

**Проверено:**
- ✅ users, services, masters, appointments, reviews — limit ограничен (max 100)

**Решение:**
1. ✅ PaginationDto с @Max(100) для limit
2. ✅ normalizePagination в контроллерах/сервисах
3. ✅ дефолт limit=20, max=100

**Файлы для создания/изменения:**
- `backend/src/common/dto/pagination.dto.ts` (создать)
- Все контроллеры с пагинацией

**Оценка:** 2-3 часа

---

### 7. Отсутствие тестов для ReferralService

**Статус:** ✅ **ВЫПОЛНЕНО** — создан referral.service.spec.ts, покрыты generateReferralCode, getOrGenerateReferralCode, getUserByReferralCode, processReferralRegistration, getReferralStats.

**Текущее состояние:**
- Файл `referral.service.spec.ts` НЕ существует
- Критический сервис для реферальной системы без тестов

**Решение:**
1. Создать `referral.service.spec.ts`
2. Покрыть тестами все методы:
   - `generateReferralCode()`
   - `getOrGenerateReferralCode()`
   - `getUserByReferralCode()`
   - `processReferralRegistration()`
   - `getReferralStats()`
3. Добавить интеграционные тесты для полного цикла

**Файлы для создания:**
- `backend/src/modules/users/referral.service.spec.ts`

**Оценка:** 4-6 часов

---

### 8. Низкое покрытие тестами модуля auth (частично исправлено)

**Статус:** ⚠️ **ЧАСТИЧНО ИСПРАВЛЕНО** - Требует улучшения

**Текущее состояние:**
- ✅ Файл `auth.service.spec.ts` существует и содержит тесты
- ✅ Есть тесты для `validateEmailPassword` с edge cases (SQL injection, длинные строки)
- ⚠️ Нет тестов для 2FA процесса (`checkPasswordWith2FA`)
- ⚠️ Нет тестов для JWT стратегий
- ⚠️ Нет тестов для Local стратегий
- ⚠️ Нет тестов для Telegram аутентификации

**Проверено:**
- ✅ Файл `backend/src/modules/auth/auth.service.spec.ts` существует (886 строк)
- ❌ Файл `backend/src/modules/auth/strategies/jwt.strategy.spec.ts` НЕ существует
- ❌ Файл `backend/src/modules/auth/strategies/local.strategy.spec.ts` НЕ существует
- ⚠️ Покрытие тестами 2FA процесса отсутствует

**Решение:**
1. Добавить тесты для `checkPasswordWith2FA` метода
2. Создать `jwt.strategy.spec.ts` для тестирования JWT стратегии
3. Создать `local.strategy.spec.ts` для тестирования Local стратегии
4. Добавить интеграционные тесты для полного цикла аутентификации
5. Добавить тесты для Telegram аутентификации

**Файлы для создания:**
- `backend/src/modules/auth/strategies/jwt.strategy.spec.ts` (создать)
- `backend/src/modules/auth/strategies/local.strategy.spec.ts` (создать)
- Расширить `backend/src/modules/auth/auth.service.spec.ts` (добавить тесты для 2FA)

**Оценка:** 6-10 часов

---

## ВАЖНЫЕ ПРОБЛЕМЫ (Приоритет 2)

### 9. Небезопасные значения по умолчанию для секретов

**Статус:** ✅ **ВЫПОЛНЕНО РАНЕЕ** — main, data-source, database.config: проверки для production, без fallback.

**Текущее состояние:**
- ✅ main: SESSION_SECRET||JWT_SECRET; в prod throw, если оба не заданы; fallback `development-insecure...` только при !sessionSecret (dev)
- ✅ database.config: в prod throw при отсутствии DB_HOST/USER/PASSWORD/NAME; password в prod без fallback `postgres`
- ✅ data-source: в prod throw до создания DataSource при отсутствии DB_*

**Проверено:**
- ✅ main, database.config, data-source — в production нет небезопасных fallback для секретов

**Решение:**
1. Удалить все fallback значения для секретов в production
2. Добавить валидацию обязательных переменных окружения при старте
3. Выбрасывать ошибку, если секреты не установлены в production
4. Обновить `env.validation.ts` для обязательных полей в production

**Файлы для изменения:**
- `backend/src/main.ts`
- `backend/src/config/data-source.ts`
- `backend/src/config/database.config.ts`
- `backend/src/config/env.validation.ts`

**Оценка:** 2-3 часа

---

### 10. Жестко закодированные URL и конфигурация

**Статус:** ✅ **ВЫПОЛНЕНО РАНЕЕ** — telegram-bot: FRONTEND_URL из env, проверка наличия.

**Текущее состояние:**
- ✅ telegram-bot: FRONTEND_URL из `configService.get('FRONTEND_URL')`, логирование при отсутствии
- ✅ main: localhost только в dev-ветке CORS; в prod — FRONTEND_URL, ADMIN_URL из env

**Проверено:**
- ✅ telegram-bot, main — жёстко закодированных URL (your-domain и т.п.) не найдено

**Решение:**
1. Вынести все URL в переменные окружения
2. Добавить валидацию URL в `env.validation.ts`
3. Удалить все жестко закодированные значения
4. Проверить весь код на наличие других жестко закодированных значений

**Файлы для изменения:**
- `backend/src/modules/telegram/telegram-bot.service.ts`
- `backend/src/config/env.validation.ts`
- Проверить весь код на наличие других жестко закодированных значений

**Оценка:** 1-2 часа

---

### 11. Неполная валидация переменных окружения

**Статус:** ✅ **ВЫПОЛНЕНО РАНЕЕ** — env.validation: ValidateIf, MinLength(32), IsUrl для FRONTEND/ADMIN, обязательные в prod.

**Текущее состояние:**
- ✅ ValidateIf для prod: DB_HOST, DB_USER, DB_PASSWORD, JWT_SECRET, SESSION_SECRET
- ✅ MinLength(32) для JWT_SECRET, MinLength(8) для DB_PASSWORD; @IsUrl для FRONTEND_URL, ADMIN_URL
- ✅ validate(): в prod requiredFields (JWT or SESSION, DB_*), throw при отсутствии

**Проверено:**
- ✅ env.validation — условная обязательность в prod, валидация формата URL и длины секретов

**Решение:**
1. Добавить обязательные поля для production (JWT_SECRET, DB_PASSWORD, SESSION_SECRET)
2. Добавить валидацию формата для URL (`@IsUrl()`)
3. Добавить проверку минимальной длины для секретов (`@MinLength(32)`)
4. Разделить валидацию для dev/prod окружений
5. Использовать `ValidateIf` для условной валидации

**Файлы для изменения:**
- `backend/src/config/env.validation.ts`

**Оценка:** 3-4 часа

---

### 12. TODO комментарии в коде

**Статус:** ✅ **ВЫПОЛНЕНО** — RegisterDto, MediaPreview, TelegramUserMessagesTab (диалог пересылки) готовы.

**Текущее состояние:**
- ✅ RegisterDto создан
- ✅ MediaPreview: GET /telegram/user/file, загрузка фото в компоненте через fetch+blob, fileReference
- ✅ TelegramUserMessagesTab: диалог выбора чата для пересылки, forwardMutation (POST .../forward), список чатов+контактов с исключением источника

**Проверено:**
- ✅ TODO для диалога пересылки удалён; заглушка toast.info убрана

**Решение:**
1. Создать `RegisterDto` для регистрации (возможно уже создан)
2. Реализовать endpoint для получения медиа файлов через Telegram API
3. ✅ Реализовать диалог выбора чата для пересылки сообщений

**Файлы для изменения:**
- `backend/src/modules/auth/controllers/auth.controller.ts` (проверить)
- `backend/src/modules/auth/dto/register.dto.ts` (проверить, возможно уже создан)
- `admin/app/telegram/components/MediaPreview.tsx`
- `admin/app/telegram/TelegramUserMessagesTab.tsx`

**Оценка:** 4-6 часов

---

### 13. Отсутствие rate limiting на критичных endpoints

**Статус:** ✅ **ВЫПОЛНЕНО** — authLimiter на /api/v1/auth/login и /api/v1/auth/register.

**Текущее состояние:**
- ✅ main: `app.use('/api/v1/auth/login', authLimiter)` и `app.use('/api/v1/auth/register', authLimiter)`

**Проверено:**
- ✅ authLimiter применяется на /auth/login и /auth/register

**Решение:**
1. Раскомментировать и применить rate limiting
2. Применить `authLimiter` на `/api/v1/auth/login`
3. Применить `authLimiter` на `/api/v1/auth/register`
4. Настроить разные лимиты для разных endpoints

**Оценка:** 1-2 часа

---

### 14. Использование @ts-ignore комментариев

**Статус:** ✅ **ВЫПОЛНЕНО** — @ts-ignore убраны; в telegram-user — as any для invoke

**Текущее состояние:**
- ✅ `@ts-ignore` в backend убраны
- В `telegram-user.controller.ts` @ts-expect-error не найден; для `client.invoke( {...} )` используются `as any` на аргументах (напр. `id: [messageIdNum] as any`, `random_id: [BigInt(...)] as any`, весь объект `as any`). Причина: типы @mtkruto для Api (messages.sendMessage, messages.deleteMessages, messages.forwardMessages и др.) строже runtime-структур (number[] vs readonly, BigInt, вложенные InputPeer и т.д.).

**Углублённо (§14):**
- Рекомендация: при обновлении @mtkruto проверить экспорт типов для messages.* и подставлять типизированные объекты; до тех пор `as any` на invoke-аргументах оставить.
- `telegram.service` (restrictChatMember, Markup.inlineKeyboard), `telegram-user-client` (Storage, DatabaseStorage) — по-прежнему точечные `as any` на границе с Telegraf/MTKruto.

**Оценка:** 3-4 часа

---

### 15. Отсутствие валидации размера входных данных

**Статус:** ✅ **ВЫПОЛНЕНО** — body parser limit 10mb (BODY_PARSER_LIMIT); @MaxLength в DTO (broadcast, telegram, contact, template, service, appointment).

**Текущее состояние:**
- Нет проверки максимального размера сообщений
- Нет проверки максимального размера файлов
- Риск DoS через большие запросы

**Решение:**
1. Добавить валидацию максимального размера сообщений
2. Добавить валидацию максимального размера файлов
3. Настроить body parser limits в NestJS

**Оценка:** 2-3 часа

---

### 16. Отсутствие транзакций в критичных операциях

**Статус:** ✅ **ВЫПОЛНЕНО** — financial.processPayment: manager.transaction для updateBonusPoints + save(transaction).

**Текущее состояние:**
- Некоторые операции не используют транзакции
- Риск race conditions и неконсистентности данных

**Проверено:**
- ⚠️ `financial.service.ts:processPayment()` - нет транзакции для обновления bonusPoints и создания transaction
- ⚠️ Некоторые операции с несколькими записями в БД

**Решение:**
1. Добавить транзакции в `processPayment()`
2. Проверить другие критические операции
3. Использовать `@Transactional()` декоратор или явные транзакции

**Оценка:** 3-4 часа

---

## УЛУЧШЕНИЯ (Приоритет 3)

### 17. Улучшение обработки ошибок

**Статус:** ✅ **ЧАСТИЧНО ВЫПОЛНЕНО** — retry для MTProto: invokeWithRetry в getMe/users.getFullUser (user-client 3×, heartbeat, guard).

**Текущее состояние:**
- ✅ Единый ErrorResponse контракт реализован
- ✅ Глобальные exception filters настроены
- ✅ Правильная обработка ValidationError[]
- ✅ Retry для Telegram MTProto: `invokeWithRetry` при FLOOD_WAIT/retryable (getMe 5×)

**Проверено:**
- ✅ `HttpExceptionFilter` правильно обрабатывает ошибки
- ✅ `ValidationExceptionFilter` предотвращает React error #31
- ✅ `ErrorMetricsService` собирает метрики ошибок
- ✅ Retry при RETRY (FLOOD_WAIT и др.): mtproto-retry.utils, user-client, heartbeat, guard

**Решение:**
1. Добавить retry логику для Telegram API вызовов
2. Добавить circuit breaker для внешних сервисов
3. Улучшить пользовательские сообщения об ошибках

**Оценка:** 4-6 часов

---

### 18. Улучшение покрытия тестами

**Статус:** 🔄 Улучшение качества

**Текущее состояние:**
- Текущее покрытие: 51.23% statements, 37.79% branches
- Целевое покрытие: >80% для критичных модулей

**Оценка:** 16-24 часа

---

### 19. Улучшение мониторинга и логирования

**Статус:** ✅ **ВЫПОЛНЕНО** — Prometheus-метрики, /metrics

**Текущее состояние:**
- ✅ Метрики: prom-client, http_requests_total, http_request_duration_seconds; MetricsInterceptor (finalize); GET /metrics (text/plain) для Prometheus/Grafana
- ✅ MetricsModule, /metrics в exclude и в /api, /api/v1; ErrorMetricsService (ошибки по ErrorCode) — был ранее

**Оценка:** 8-12 часов

---

### 20. Оптимизация запросов к БД

**Статус:** ✅ **ЧАСТИЧНО ВЫПОЛНЕНО** — медленные запросы, N+1 аудит

**Текущее состояние:**
- ✅ Медленные запросы: maxQueryExecutionTime (DB_SLOW_QUERY_MS, по умолч. 5000 в prod), TypeOrmSlowQueryLogger (logQuerySlow) в production
- ✅ Аудит N+1: appointments, notifications — relations/один запрос; массовые find в циклах не выявлены
- Индексы — ранее; при необходимости: подписка на query runner для метрик

**Оценка:** 6-8 часов

---

### 21. Улучшение документации API

**Статус:** ✅ **ЧАСТИЧНО ВЫПОЛНЕНО** — теги Swagger

**Текущее состояние:**
- ✅ Swagger: добавлены теги health, metrics, settings, financial, reviews, audit, templates, contact-requests, telegram, scheduler (к auth, appointments, services, masters, users, notifications, analytics)
- Контроллеры с @ApiTags, @ApiOperation — были; при необходимости: @ApiBody, DTO-схемы

**Оценка:** 4-6 часов

---

## BACKEND

### Текущее состояние

**Модули:**
- ✅ Хорошая модульная структура
- ✅ Правильное использование TypeORM
- ✅ Настроена валидация и обработка ошибок
- ✅ Архитектурные принципы — message.includes в mapper (§2)
- ✅ TypeScript strict: true, strictNullChecks, noImplicitAny (сборка OK)
- ⚠️ any: telegram-bot типизирован (§5); в notifications, telegram-user-client, auth, users, http-exception.filter, telegram-user.controller и др. — остаются; рекомендуются DTO и unknown/приведения
- ✅ Пагинация — appointments, reviews; notifications sendBroadcast .take(5000) (§3)

**Зависимости:**
- NestJS 10.3.0
- TypeORM 0.3.17
- PostgreSQL через TypeORM
- Redis для кэширования
- Telegram MTProto через @mtkruto/node

---

## FRONTEND

### Admin (Next.js)

**Текущее состояние:**
- ✅ Next.js 14 с App Router
- ✅ TypeScript
- ✅ Tailwind CSS

### Telegram App (React + Vite)

**Текущее состояние:**
- ✅ React 18
- ✅ Vite для сборки
- ✅ TypeScript

### Landing (React + Vite)

**Текущее состояние:**
- ✅ React 18
- ✅ Vite для сборки
- ✅ Tailwind CSS

---

## План выполнения

### Фаза 1: Критические исправления (1-2 недели)

1. ✅ Исправить 2FA аутентификацию (4-6 часов) — checkPassword из @mtkruto/node, самописный SRP удалён, tssrp6a удалена
2. ✅ Исправить нарушение архитектурных принципов (2-3 часа) — isRequire2faActionError в mapper, убраны message.includes из handler и auth.service
3. ✅ Добавить пагинацию в критичные запросы (3-4 часа) — appointments, reviews, notifications.sendBroadcast .take(5000)
4. ✅ Добавить валидацию максимального limit (2-3 часа) — PaginationDto, normalizePagination(max 100) в 5 модулях
5. ✅ Добавить тесты для ReferralService (4-6 часов) — referral.service.spec.ts
6. ⚠️ Улучшить покрытие тестами модуля auth (6-10 часов) — частично; нужны 2FA, JWT, Local

**Итого:** 21-32 часа

### Фаза 2: Важные улучшения (2-3 недели)

7. ✅ Включить TypeScript strict mode постепенно — strict: true, strictNullChecks, noImplicitAny (сборка OK)
8. ✅ Заменить все any типы в telegram-bot — options/keyboard (Types.ExtraReplyMessage, { reply_markup }), keyboard/currentRow/keyboardButtons (unknown[][], unknown[]; Markup.inlineKeyboard as never)
9. ✅ Убрать @ts-ignore комментарии (3-4 часа) — 6 мест: 5 сняты, 1 @ts-expect-error (messages.sendMessage)
10. ✅ Исправить небезопасные значения по умолчанию для секретов (2-3 часа) — main, data-source, database.config
11. ✅ Убрать жестко закодированные URL (1-2 часа) — FRONTEND_URL в telegram-bot
12. ✅ Улучшить валидацию переменных окружения (3-4 часа) — env.validation: ValidateIf, MinLength, IsUrl
13. ✅ Реализовать TODO комментарии (4-6 часов) — RegisterDto, MediaPreview (endpoint + загрузка), TelegramUserMessagesTab (диалог пересылки) готовы
14. ✅ Добавить rate limiting на критичные endpoints (1-2 часа) — authLimiter на /auth/login, /auth/register
15. ✅ Добавить валидацию размера входных данных (2-3 часа) — body 10mb, @MaxLength в DTO
16. ✅ Добавить транзакции в критичные операции (3-4 часа) — processPayment: manager.transaction для bonusPoints + Transaction
17. ✅ Улучшить обработку ошибок (4-6 часов) — retry для MTProto getMe (invokeWithRetry)

**Итого:** 35-47 часов

### Фаза 3: Дополнительные улучшения (3-4 недели)

18. 🔄 Улучшить покрытие тестами (16-24 часа)
19. ✅ Улучшить мониторинг — prom-client, /metrics, http_requests_total, http_request_duration_seconds
20. ✅ Оптимизировать запросы к БД — maxQueryExecutionTime, TypeOrmSlowQueryLogger, аудит N+1
21. ✅ Улучшить документацию API — теги Swagger (health, metrics, settings, financial, reviews, audit, templates, contact-requests, telegram, scheduler)

**Итого:** 34-50 часов

---

## Итоговая оценка

**Общее время:** 90-129 часов (11-16 рабочих дней)

**Приоритет выполнения:**
1. Критические проблемы (Фаза 1) - немедленно
2. Важные проблемы (Фаза 2) - в течение месяца
3. Улучшения (Фаза 3) - по возможности

---

## Метрики успеха

- ✅ 2FA аутентификация — checkPassword из @mtkruto/node, самописный SRP удалён
- ✅ Архитектурные принципы соблюдаются — message.includes убраны, проверки в telegram-error-mapper
- ✅ Все запросы имеют пагинацию — appointments, reviews; notifications sendBroadcast с .take(5000)
- ✅ TypeScript strict: true, strictNullChecks, noImplicitAny, noUnusedLocals (сборка OK)
- ❌ Нет использований any типа
- ⚠️ Покрытие тестами >80% для критичных модулей
- ✅ Rate limiting на auth — authLimiter на /auth/login, /auth/register
- ✅ Транзакции в processPayment — manager.transaction для bonusPoints + Transaction
- ✅ Валидация размера входных данных — body 10mb, @MaxLength в DTO
- ✅ Метрики: /metrics (Prometheus), http_requests_total, http_request_duration_seconds; ErrorMetricsService
- ✅ Документация API: Swagger, теги для основных модулей

---

## Проблемы из предыдущих планов, перенесенные в этот план

1. **Низкое покрытие тестами модуля auth** — частично исправлено; требуются тесты для 2FA, JWT, Local (§8)
2. **Небезопасные значения по умолчанию для секретов** — ✅ §9: main, data-source, database.config без fallback в prod
3. **Жестко закодированные URL** — ✅ §10: telegram-bot, FRONTEND_URL из env
4. **Неполная валидация переменных окружения** — ✅ §11: env.validation, ValidateIf, MinLength, IsUrl
5. **TODO комментарии в коде** — ✅ §12: RegisterDto, MediaPreview, TelegramUserMessagesTab
6. **Улучшение обработки ошибок** — ✅ §17: retry для MTProto (invokeWithRetry)

## Новые проблемы, обнаруженные при анализе

1. **Нарушение архитектурных принципов** — ✅ §2: message.includes в mapper, убраны из handler и auth.service
2. **Отсутствие пагинации** — ✅ §3: appointments, reviews, notifications.take(5000)
3. **TypeScript strict mode** — ✅ strict: true, strictNullChecks, noImplicitAny
4. **Множественные использования any** — ⚠️ telegram-bot типизирован (§5); остаются в др. модулях
5. **Отсутствие валидации limit** — ✅ §6: PaginationDto, normalizePagination(max 100)
6. **Отсутствие rate limiting** — ✅ §13: authLimiter на /auth/login, /auth/register
7. **Использование @ts-ignore** — ✅ §14: убраны; as any на границе @mtkruto
8. **Отсутствие транзакций** — ✅ §16: processPayment с manager.transaction

---

## Следующие шаги

1. **Приоритет:** Улучшение покрытия тестами (§8 auth, §18) — в последнюю очередь
2. Постепенная замена `error: any` / `e: any` на `unknown` и getErrorMessage — остаются: (пусто). ✅ выполнено: users, settings, appointments (8), scheduler (2), telegram.service (1), scheduled-messages (2), telegram-heartbeat (2), telegram-connection-monitor (1), session-encryption (2), telegram-session (3+1 load), telegram-session.guard (3+1+connectError), auth.controller (2), telegram.controller (6+getChat _error), auth.service (15+3: safeDisconnectClient, requestPhoneCode invokeError, checkQrTokenStatus referral), telegram-user.controller (16: …), telegram-user-client.service (23: …). 23.01: доисправлены оставшиеся 5× (auth 3, telegram.controller 1, guard 1).
3. Сокращение any в notifications, telegram-user-client, auth, users и др. (DTO, unknown). ✅ частично: users.controller, settings.controller, masters, scheduled-messages, auto-replies, scheduler, telegram (setChatPermissions), websocket, audit, notifications, users, settings.service, telegram-session, scheduled-messages, templates — 23.01; jwt-auth.guard (handleRequest err, user, info → unknown), entities (user preferences, notification payload → Record<string, unknown>), telegram.controller (userInfo: unknown, TelegramChatFromApi для chatInfo, убраны (chatInfo as any).xxx) — 23.01. Остаются: telegram-user-client, auth (expressRequest, user: any), telegram (deleteMyCommands, getUserProfilePhotos, getChat, getChatMember, telegram.controller (telegramChatsService as any).telegramChatRepository в 2 местах), filters (http-exception, validation-exception — exceptionResponse as any), safe-logger, sensitive-data-masker и др.
4. §17: circuit breaker для внешних сервисов; улучшение пользовательских сообщений об ошибках
5. Проводить code review после каждой фазы
6. Обновлять REPORT.md по мере выполнения задач
7. Тестировать изменения в dev перед production
